...
We fit the model: \texttt{y $\sim$ block + treatment} (instead of fitting \texttt{y $\sim$ treatment + block}). Since want to check whether the treatment has any influence on the response after having controlled for the variation between blocks (only important with \texttt{summary}).
As it is an unbalanced data set, we use \textit{drop1} to analyse the data, such that we get the sum of squares.
\begin{lstlisting}
