...
where $\alpha_i$ is the effect of the $i$ samples, it is also called a \textbf{random effect}. There are no longer any sideconstraints on $\alpha_i$. Sometimes, such models are also called \textbf{variance components models}. Let us inspect some properties of the model.
...
Confidence intervals are often larger than with fixed effect models, as we now try to make a statement about a larger population and not only about the measured samples. In general, variances are "difficult" to estimate in the sense that we need a lot of observations to have some reasonable accuracy. To verify our model assumptions we can again use QQ-plots, but we have to also plot $\sigma_\alpha^2$:
\begin{lstlisting}
...
If we would fit a normal one-way ANOVA model, we could estimate $\sigma^2_\alpha$ by $\frac{MS_A - MS_E}{N}$.
