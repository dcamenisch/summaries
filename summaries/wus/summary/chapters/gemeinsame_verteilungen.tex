...
\Title{Diskrete gemeinsame Verteilungen}
...
\Satz Die \textbf{Randverteilung} gegeben durch:
$$\forall z \in W_i. \; \Pm[X_i = z] = \sum_{x_1, ..., x_{i-1}, x_{i+1},...,x_n} p(x_1, ..., x_{i-1}, z, x_{i+1},...,x_n)$$

\Satz Der Erwartungswert definiert als:
...
\Title{Stetige gemeinsame Verteilungen}
...
\Satz Seien $X_1,...,X_n$ stetige ZV mit einer gemeinsamen Dicht $f$ und $\phi: \R^n \mapsto \R$. Dann ist der Erwartungswert definiert als:
$$\E[\phi(X_1, ..., X_n)] = \int_{-\infty}^\infty ... \int_{-\infty}^\infty \phi(x_1, ...,x_n) \cdot f(x_1,...,x_n) dx_n...dx_1$$

\Satz Falls $X_1,...,X_n$ eine gemeinsame Dichte $f$ besitzt. Dann ist die Randverteilung:
\begin{align*}
    f_i(z) =& \int_{(x_1, ..., x_{i-1},x_{i+1},...x_n) \in R^{n-1}} \\ 
    & \quad f(x_1,...,x_{i-1},z,x_{i+1},...,x_n)dx_n...dx_{i+1}dx_{i-1}...dx_1
\end{align*}

\Satz Seien $X_1,...,X_n$ stetige ZV mit Dichten $f_1,..., f_n$, dann sind die folgenden Aussagen äquivalent:
\begin{enumerate}
    \item $X_1,...,X_n$ sind unabhängig
