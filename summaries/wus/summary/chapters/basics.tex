...
\begin{tcolorbox}
    ...
    \begin{enumerate}
        \item $\Pm[\Omega] = 1$
        \item $\Pm[A] = \sum_{i=1}^\infty \Pm[A_i]$, falls $A = \bigsqcup_{i = 1}^\infty A_i$
    \end{enumerate}
\end{tcolorbox}
...
\Satz \textbf{Monotonie}: Seien $A, B \in \F$ dann gilt 
$$A \subseteq B \Rightarrow \Pm[A] \leq \Pm[B]$$

\Satz \textbf{Union Bound}: Seien $A_1, A_2, ... \in \F$ dann gilt 
...
$$\Pm[A | B] = \frac{\Pm[A \cap B]}{\Pm[B]}, \text{ wobei } \Pm[A \cap B] = \Pm[A | B] \Pm[B]$$

\Satz \textbf{Totale Wahrscheinlichkeit}: Sei $A_1, ..., A_n \in \F$ eine Partition von $\Omega$ mit $\Pm[A_i] > 0$ für alle $1 \leq i \leq n$. Dann gilt:
$$\forall B \in \F. \quad \Pm[B] = \sum_{i = 1}^n \Pm[B | A_i] \cdot \Pm[A_i]$$

\Satz \textbf{Formel von Bayes}: Sei $A_1, ..., A_n \in \F$ eine Partition von $\Omega$ mit $\Pm[A_i] > 0$ für alle $1 \leq i \leq n$. Für jedes $B \in \F$ mit $\Pm[B] > 0$ gilt:
...
\begin{tcolorbox}
    ...
    $$\Pm[A \cap B] = \Pm[A] \cdot \Pm[B] \text{ woraus folgt } \Pm[A | B] = \Pm[A]$$
\end{tcolorbox}
